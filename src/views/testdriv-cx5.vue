<template>
  <div>
    <Card title="Test Drive CX-5" noborder>
      <div class="btnExport">
      ...
      <vue-good-table :columns="columns" styleClass="vgt-table bordered centered" :rows="paginatedFilteredData"
        ...
        <template v-slot:table-row="props">
          ...
          <span v-if="props.column.field == 'modelsInterested'">{{ props.row.modelsInterested }}</span>
  ...
  data() {
    ...
    return {
      ...
      columns: [
        ...
        // { label: "วันที่นัดหมายทดลองขับ", field: "selectedDate", width: "250px" },
        //{ label: "ช่วงเวลาที่นัดหมาย", field: "selectedTime", width: "250px" },
      ...
      if (result) {
        ...
        try {
          const response = await axios.put(this.urlEnv + '/lead-form/test-drive-status', {
            ...
            campaignName: "new-cx-5-pre-register",
          });
