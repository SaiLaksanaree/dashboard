...
// import Multipule from "@/src/view/form/form-validation/Multipule";
import Multipule from "./forms/form-validation/Multipule.vue";
  ...
  methods: {
    ...
    async syncData() {
      try {
        ...
        const { nameThai, email, phone, mapUrl } = response.data;
    ...
    async syncDataResetPassword() {
      try {
        ...
        // ตรวจสอบว่าได้ email มาหรือไม่
        if (!email) {
          console.error('Email not found in JWT payload');
          return; // หยุดทำงานถ้าไม่มี email
        }
        ...
        const response = await axios.request(config);
        const { nameThai, email: responseEmail, phone, mapUrl } = response.data;
